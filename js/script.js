window.addEventListener("DOMContentLoaded", function () {
    ...
    const inputAllValidation = () => {
        ...
        calcBlock.addEventListener('input', (event) => {
            let target = event.target;
            target = event.target.closest('.calc-item');
            if (target) {
                let text = inputCalc.value;
                inputCalc.value = text.replace(/\D/g, '');
            }
        });
    };
    inputAllValidation();
    ...
    const command = () => {
        ...
        console.log('commandRow: ', commandRow);
        const imgCommand = document.querySelectorAll('.command__photo');
        console.log('imgCommand: ', imgCommand);
